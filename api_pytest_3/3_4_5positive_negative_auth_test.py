...
# Задание 3_4_5 Негативные тесты на авторизацию пользователя
    ...
    def test_negative_auth_user(self, condition):               # в тесте будем вызывать пременную condition со значениями словаря exclude_params
        data = {                                                # далее идёт часть кода из "3_4", до второго запроса
            'email': 'vinkotov@example.com',
            'password': '1234'
        }
        response = requests.post("https://playground.learnqa.ru/api/user/login", data=data)
...
# далее идёт код со значениями из словаря exclude_params, где мы передаём в запрос только параметр с куки или токеном
        if condition == 'no_cookie':
            response0 = requests.get("https://playground.learnqa.ru/api/user/auth", headers={'x-csrf-token': token})
        else:
            response0 = requests.get("https://playground.learnqa.ru/api/user/auth", cookies={'auth_sid': auth_sid})
